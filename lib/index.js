...
function registerGate(ctx, state) {
  ...
  // tools/post-execute（waterfall）：catalog 或 read 任一成功即放行（防「无合适模式」死锁）。
  const offPost = ctx.on("tools/post-execute", async (exec, result, next) => {
    try {
      if ((exec.name === "jdpatterns_catalog" || exec.name === "jdpatterns_read") && exec.agent && exec.agent.id) {
        const success = result && result.isError === false
          && Array.isArray(result.content) && result.content.length > 0
          && !(result.value && typeof result.value === "object" && result.value.error);
        if (success) state.gatedSessions.add(exec.agent.id);
      }
    } catch { /* 记录失败不影响调用 */ }
    return next();
  });
